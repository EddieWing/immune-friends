...
func update(sim,delta):
 ...
 for b in sim.blood:
  ...
  var state=states.get(b.id,{"look":Vector2.ZERO,"target":-1,"emotion":"calm","fear":0.0,"relief":0.0})
  ...
  state.emotion="scared" if state.fear>0 else ("relieved" if state.relief>0 else ("surprised" if not nearest.is_empty() else "calm"))
...
func draw(canvas,b,time):
 ...
 for side in [-1,1]:
  ...
  if emotion=="relieved" or (blink and emotion=="calm"):
